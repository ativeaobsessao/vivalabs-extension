...
async function initDb() {
  ...
  await query(`
    CREATE TABLE IF NOT EXISTS funnel_nodes (
      ...
      tipo       TEXT NOT NULL CHECK (tipo IN ('advertorial','tsl','vsl','quiz','whatsapp','checkout','upsell')),
  ...
  // AUDITORIA #07: a extensão oferece "upsell" como tipo de etapa do funil, mas o CHECK
  // constraint de funnel_nodes.tipo (criado antes de "upsell" existir) não permitia esse valor —
  // salvar uma etapa "upsell" violava a constraint, /api/funis/salvar-node respondia 500, e o
  // loop de salvamento da extensão tratava isso como falha silenciosa só daquela etapa.
  // CREATE TABLE IF NOT EXISTS acima só ajuda bancos NOVOS — não altera uma tabela que já
  // existe. Esta migração corrige o CHECK em bancos já existentes; seguro rodar sempre (só
  // mexe na constraint se "upsell" ainda não estiver na lista permitida).
  await query(`
    DO $$
    DECLARE
      con_name text;
    ...
    BEGIN
      ...
      WHERE con.conrelid = 'funnel_nodes'::regclass AND con.contype = 'c'
      LIMIT 1;

      IF con_name IS NOT NULL AND con_def NOT LIKE '%upsell%' THEN
        EXECUTE format('ALTER TABLE funnel_nodes DROP CONSTRAINT %I', con_name);
        ALTER TABLE funnel_nodes
          ADD CONSTRAINT funnel_nodes_tipo_check
          CHECK (tipo IN ('advertorial','tsl','vsl','quiz','whatsapp','checkout','upsell'));
      END IF;
    END $$;
  `);
...
const TIPOS_ORDEM = ["advertorial", "tsl", "vsl", "quiz", "whatsapp", "checkout", "upsell"];
